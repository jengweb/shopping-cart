...
type PaymentInterface interface {
	...
	ConfirmPayment2(orderID int, paymentdetail PaymentDetail) (string, error)
}
...
func (service PaymentService) ConfirmPayment2(orderID int, paymentdetail PaymentDetail) (string, error) {
	transactionId, err := service.BankGateway.Payment(paymentdetail)
	if err != nil {
		log.Printf("BankGateway.Payment internal error %s", err.Error())
		return "", err
